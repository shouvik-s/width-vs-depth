...
def iterative_map(func, x_arr, n):
    # kept for compatibility with code0.py; evaluated in closed form (no round-off amplification)
...
class ShallowMLP(nn.Module):
    def __init__(self, w=WIDTH, n=MAP_DEGREE):
        ...
        # --- deterministic initialization of the weights and biases
        with torch.no_grad():
...
class DeepMLP(nn.Module):
    def __init__(self, w=2, n=MAP_DEGREE, noise=1e-2):
        super().__init__()
        # n ReLU layers TOTAL: first hidden layer plus (n-1) hidden layers
